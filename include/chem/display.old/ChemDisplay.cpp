...
//					 matchpos-> get_end_pos()-> dim[PEPPOS_X],matchpos-> get_end_pos()-> dim[PEPPOS_Y], NULL);
	...
	if (matchpos-> get_rotation() >3) {
		...
		M2coords.posx -= 3;
		// -------------- M2
		if (matchpos-> getM2()!=NULL) {
			gfx.color(&m1_col);
			gfx.cprintg((const char*) "M2");
...
//-------------------------------
void ChemDisplay::draw_enz(ChemScreen *screen, ChemEnzyme *enz){
	if (screen==NULL) return;
	//if (enz==NULL) return;

	char str[128];
	if (enz==NULL) {
		sprintf(str, "NULL enz");
		gfx.printg(str);
	} else {
		Molecule 	*m = enz-> get_mole();
		ChemFunc 	*f = enz-> get_func();
		sprintf(str, "enz[0x%zX]: Mole[0x%zX] Func[0x%zX]",
				(long unsigned int) enz,
				(long unsigned int) m,
				(long unsigned int) f);
		gfx.printg(str);

		if (f!=NULL) {
			sprintf(str, "func[%s]", f->name.get());
			gfx.printg(str);
		}

		if (m!=NULL) {
			draw_mole(screen, &screen->coords, m, 50, 100, 0, false);
...
void ChemDisplay::draw_conc_displ(ChemScreen *screen, ChemConcDisplay *conc_display){
	...
	if (c>1) {
		...
		float scaley = 0;
		if (max!=min)
			scaley =  - conc_display->coords.scaley / (max-min);
	...
	char 	msg[128];
//	sprintf(msg, "Conc[0x%zX] Mole[0x%zX] Vol[0x%zX]", (long unsigned int) cp, (long unsigned int) conc_display->get_mole(), (long unsigned int) conc_display->get_vol());	gfx.text(msg,  x-sx, y-sy);
	sprintf(msg, "Mole[0x%zX] Vol[0x%zX]", (long unsigned int) conc_display->get_mole(), (long unsigned int) conc_display->get_vol());
															gfx.text(msg,  x-sx, y-sy);
	sprintf(msg, "Conc[0x%zX] (%d/%d samples)", (long unsigned int) cp, c, conc_display->buf.getsize() );	gfx.text(msg,  x-sx, y+sy+FONT_HEIGHT);


	//sprintf(msg, "(%d samples)", c);						gfx.text(msg,  x-sx, y+sy);
...
void ChemDisplay::draw_molelist(ChemScreen *screen, Concentration_CLI *cli, ChemMolelistDisplay *molelistDisplay, bool mouseclick) {
	...
	while (true) {
		...
		if ((mole_item!=NULL) && (mole_item->item !=NULL)) {
			...
			if ((cli->local_vm.conc!=NULL) && (mole_item->item == cli->local_vm.conc-> getmole())) {
				txtcol.set(0,100,100);
			}

			if (mole_item->item ==  cli->local_vm.mole) {
...
//				if (hit!=NULL) { selected_conc = hit; }
			...
			*/
			//Concentration_VM  *vm = cli-> get_selected_vm();
			//if (vm!=NULL) {
				if (m==cli->local_vm.mole) {
					txtcol.set(0,100,100);
				}
				if (conc_item->item ==cli->local_vm.conc)
					txtcol = conclistDisplay->selcol;
			//}
...
//-------------------------------------------
//-------------------------------------------
void	ChemDisplay::draw_title_bar(ChemScreen *screen, Concentration_CLI *cli) {
	if (screen==NULL) return;
	if (cli==NULL) return;
...
*******************/
...

	{
		ChemStep t = 0;
		if (cli->world!=NULL)
			t = cli->world->chem_engine.get_tick();
		// draw conc disp
		// ------------------
