...
// ---------------------------------------------------
// simple noop
int 	eng_noop(ChemEngine *eng, Concentration_VM *vm, int argc, char **argv){
	PRINT("== PING ==\n");
	PRINT(": argc[%d]", argc);
...
// ---------------------------------------------------
// dump
int 	eng_dump(ChemEngine *eng, Concentration_VM *vm, int argc, char **argv){
	PRINT("== ENG ==\n");
	DUMP(eng)
	PRINT("== VM ==\n");
	DUMP(vm)
...
// ---------------------------------------------------
int 	eng_addenz(ChemEngine *eng, Concentration_VM *vm, int argc, char **argv){
	if (eng==NULL) { printf("vm is NULL\n"); return -10; }
	if (vm==NULL) { printf("vm is NULL\n"); return -11; }
	Molecule *m = vm->mole;
	if (m==NULL) { printf("mole is NULL\n"); return -20; }
	if (argc<1) { printf("no ChemFunc provided (use 'eng' to list)\n"); return -21; }
